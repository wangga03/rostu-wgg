...
def PIDPosisi3(x_target,y_target):
    global pos_x,pos_yaw,i,pre_yaw,compass,enc1,flag,xp,set_zero_compass,start_pos,pos_x,pos_y,set_point_arah
    current_x = pos_x
    current_y = pos_y
    delta_x = abs(current_x - x_target)
...
#PID posisi x
    ...
    # if (set_point_arah >= 180 and set_point_arah <360):
    #     set_point_arah_hitung = set_point_arah - 180
    # PIDPosisi3if (compass >= 180 and compass <360) :
    #     compas_hitung = compass - 180
    ...
    if (flagPIDPosisi3==1 and radius <= 40) :
        xp = 0 
        # set_point_arah = set_zero_compass
        flag = 2
...
def ngitung2():
    global datax, datay, jarak, p, integral, outy
...
def kejar_bola():
    ...
    while(ulang):
        ...
        if(outy < 0.08) : ambil_bola()
        if (outy >= 0.08) : stop_bola()
        ngitung()
        ngitPIDPosisi3ung2()
...
def sub_inisiasi() :
    ...
    global rate
    global kick_pub
    global grip_pub,set_zero_compass,target_x,target_y
    ...
    kick_pub = rospy.Publisher('topic_kick',UInt73 
8, queue_size=10)
    grip_pub = rospy.Publishtarget_x,target_
    rospy.sleep(30)
    ...
    while not rospy.is_shutdown():
        #PID_posisi(600)
        # PID_posisi2(target_x,target_y)
        PIDPosisi3(target_x,target_y)
       # kejar_bola()
        #kick()
        #rospy.sleep(2)
