...
import rospy
from std_msgs.msg import Int16, String, UInt16, Float32
datax = 411
datay = 240
datasdt = 280
datajrk = 100
...
def ngitung():
    ...
    derivative = (error_sdt/1000) - pre_error_sudut
...
def kejar_bola():
    ...
    hello_str = "{0} 0 {1} {2} {3} {4} {5} {6}".format (float(outy), float(outw), int(jarak), int(datajrk), int(datay), int(datasdt), int(datatof))
    pub.publish(hello_str)
    rospy.loginfo(hello_str)
